	...
	<div class="modal-body">
		<form id="fm" method="post" class="form-horizontal">
			...
			<div class="control-group">
				...
				<div class="controls">
					<input type="text" id="name" name="name"  not-null="true" desc="角色名" maxlength="64">
				</div>
			</div>
			<c:forEach items="${menus}" var="item" varStatus="vs">
			<div class="control-group">
				<c:if test="${vs.index == 0}">
					<label class="control-label" for="menu_ids${vs.index}">权限</label>
				</c:if>
				<div class="controls">
					<label id="menu_ids${vs.index}" class="checkbox"> 
						<input name="menu_ids" type="checkbox" value="${item.id}" v="${item.id}">${item.name}
					</label>
				</div>
			</div>
			</c:forEach>
...
<script type="text/javascript">
	...
	function toPage(current) {
		location.href = "${pageContext.request.contextPath}/manage/goRoleManage.do?menuIndex=${menuIndex}&current=" + current;
	}
	...
	function openAdd() {
		...
		url = "addRole.do";
		$("#fm").formClear();
		var cks = $("input[name=menu_ids]");
		for(var i=0; i<cks.length; i++) {
			$(cks[i]).attr("value", $(cks[i]).attr("v"));
		}
	...
	function openEdit(id) {
		...
		$.post(
			"${pageContext.request.contextPath}/manage/queryRoleDetail.do",
			{"id":id},
			function(res) {
				if(res.code) {
					$("#fm").formClear();
					...
					if(data.menus != null && data.menus.length > 0) {
						for(var i=0; i<data.menus.length; i++) {
							$("input[name=menu_ids][v="+data.menus[i].id+"]").attr("checked",true);
						}
					}
					$("#dialog").modal({show:true});
